...
import { Upload, Button, Card, message, Progress, Alert, Space, Typography, Divider } from 'antd';
...
const ParticipantImport = ({ onImportSuccess }) => {
  ...
  const handleClearAll = async () => {
    try {
      await axios.delete('/api/participants/clear');
      message.success('所有參賽者資料已清空！');
      
      if (onImportSuccess) {
        onImportSuccess();
      }
    } catch (error) {
      message.error('清空資料失敗');
    }
  };
            ...
            description={
              <div>
                ...
                <ul>
                  ...
                  <li><Text code>組別</Text> - 工場組 或 展能組（必填）</li>
                  <li><Text code>單位</Text> - 參賽者所屬單位（選填）</li>
                </ul>
                <Paragraph>
                  <Text type="warning">
